...
public extension Date {
    ...
    func isLaterThan(_ comparedDate: Date) -> Bool {
        let result = Calendar.current.compare(self, to: comparedDate, toGranularity: .day)
        return result == .orderedAscending
    }

    func isLaterOrEqualThan(_ comparedDate: Date) -> Bool {
        let result = Calendar.current.compare(self, to: comparedDate, toGranularity: .day)
        return result == .orderedAscending || result == .orderedSame
    }

    func isEarlierThan(_ comparedDate: Date) -> Bool {
        let result = Calendar.current.compare(self, to: comparedDate, toGranularity: .day)
        return result == .orderedDescending
    }

    func isEarlierOrEqulThan(_ comparedDate: Date) -> Bool {
        let result = Calendar.current.compare(self, to: comparedDate, toGranularity: .day)
