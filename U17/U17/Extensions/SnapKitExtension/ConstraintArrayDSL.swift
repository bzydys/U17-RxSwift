...
public struct ConstraintArrayDSL {
            ...
            for (i, v) in self.array.enumerated() {
                v.snp.makeConstraints({ (make) in
                    ...
                    if let pageCount = pageCount {
                        make.width.equalTo(tempSuperView)
                            .multipliedBy(1 / CGFloat(pageCount))
                            .offset(-(leadSpacing + fixedSpacing / CGFloat(pageCount)))
                        make.height.equalTo(tempSuperView)
                    }
                })
                prev = v;
            }
    ...
    ///   - tailSpacing: the spacing after the last item and the container
    public func distributeViewsAlong(axisType:NSLayoutConstraint.Axis,
                                     fixedSpacing:((_ index:Int)->(CGFloat)),
                                     leadSpacing:CGFloat = 0,
                                     tailSpacing:CGFloat = 0,
                                     pageCount:CGFloat? = nil) {
        ...
        if axisType == .horizontal {
            ...
            for (i, v) in self.array.enumerated() {
                v.snp.makeConstraints({ (make) in
                    if prev != nil {
                        make.width.equalTo(prev!)
                        make.left.equalTo((prev?.snp.right)!).offset(fixedSpacing(i))
                        if (i == self.array.count - 1) {//last one
                            make.right.equalTo(tempSuperView).offset(-tailSpacing);
                        }
                    }else {
                    ...
                    if let pageCount = pageCount {
                        make.width.equalTo(tempSuperView)
                            .multipliedBy(1 / CGFloat(pageCount))
                            .offset(-(leadSpacing + fixedSpacing(i) / CGFloat(pageCount)))
                        make.height.equalTo(tempSuperView)
                    }
                })
                prev = v;
            ...
            for (i, v) in self.array.enumerated() {
                v.snp.makeConstraints({ (make) in
                    if prev != nil {
                        make.height.equalTo(prev!)
                        make.top.equalTo((prev?.snp.bottom)!).offset(fixedSpacing(i))
                        if (i == self.array.count - 1) {//last one
                            make.bottom.equalTo(tempSuperView).offset(-tailSpacing);
                        }
                    }else {
