...
- (void)setProgressStyle:(DKProgressStyle)progressStyle {
    ...
    if (progressStyle & DKProgressStyle_Gradual) {
        ...
        gl.colors = @[(__bridge id)[UIColor colorWithRed:160/255.0 green:98/255.0 blue:255/255.0 alpha:1.0].CGColor, (__bridge id)[UIColor colorWithRed:100/255.0 green:52/255.0 blue:232/255.0 alpha:1.0].CGColor];
        gl.locations = @[@(0), @(1.0f)];
        gl.frame = CGRectMake(0, 0, 0, _lineHeight);
        _gradientLayer = gl;
        [self addSublayer:gl];
    }
}
...
- (void)progressAnimationCompletion {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.gradientLayer.frame = CGRectMake(0, 0, 0, self.lineHeight);
    });
}
